...
def insert_document(conn: Connection, doc: Document, is_holdout: bool = False):
...
def insert_entity(
    ...
    embedding_str = (
        str(entity_embedding.embedding) if entity_embedding.embedding else None
    )
    ...
    with conn.cursor() as cursor:
        ...
        """,
            (
                extraction_id,
                entity.id,
                entity.canonical_mention,
                entity.aliases,
                entity.entity_type,
                entity.description,
                entity.valid,
                Jsonb(entity.validation_errors)
                if entity.validation_errors
                else Jsonb([]),
                entity_embedding.text,
                embedding_str,
            ),
        )
...
def insert_triple(
    ...
    """Inserts a triple."""
    embedding_str = (
        str(triple_embedding.embedding) if triple_embedding.embedding else None
    )
    ...
    with conn.cursor() as cursor:
        ...
        """,
            (
                ...
                embedding_str,
                triple.valid,
                Jsonb(triple.validation_errors)
                if triple.validation_errors
                else Jsonb([]),
            ),
        )
        ...
        with conn.cursor() as cur:
            ...
            """,
                (
                    ...
                    generator_usage.cached_tokens,
                    generator_usage.cost,
                    latency_ms,
                    status,
                    error_message,
                ),
