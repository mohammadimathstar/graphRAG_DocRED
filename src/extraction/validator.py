...
ASYMMETRIC_NO_SELFLOOP = frozenset({
    ...
    "employer", "founded by", "capital", "capital of",
    "head of government", "head of state", "chairperson",
    "parent organization", "parent taxon", "subclass of",
    "headquarters location", "residence", "work location",
})
...
def check_evidence_grounded(t: Triple, s: Entity, o: Entity, text: str) -> Optional[str]:
...
def check_duplicate_entities(doc: ExtractionDocument) -> Optional[str]:
    ...
    mentions = [e.canonical_mention.lower() for e in doc.entities if e.canonical_mention]
...
def check_orphan_entities(doc: ExtractionDocument) -> Optional[str]:
    ...
    referenced_ids = {t.subject_id for t in doc.triples} | {t.object_id for t in doc.triples}
...
def soft_validate(doc: ExtractionDocument, source_text: str) -> ExtractionDocument:
    ...
    for e in doc.entities:
        errors = [msg for msg in (check(e, source_text) for check in ENTITY_CHECKS) if msg]
    ...
    for t in doc.triples:
        ...
        errors = [msg for msg in (check(t, subj, obj, source_text) for check in TRIPLE_CHECKS) if msg]
        t.validation_errors = errors
        t.valid = not errors

        if errors:
            print(f"Triple '{t.subject_id}-{t.relation}-{t.object_id}' failed {len(errors)} checks: {errors}")
