...
from src.db.ingest import (
    insert_document, insert_extraction, insert_evaluation, insert_entity,     
    insert_triple
)
...
def process_single_document(
    ...
    try:
        ...
        # B. Extract & Validate
        extraction_result = extractor.extract(doc_obj.text, document_id=doc_obj.document_id) 
        extraction_result.data = soft_validate(extraction_result.data, source_text=doc_obj.text) 
        ...
        for triple, triple_emb in zip(extraction_result.data.triples, triple_embs):
            ...
            if not subject_uuid or not object_uuid:
                print(f"  [WARN] Skipping hallucinated triple: {triple.subject_id} -> {triple.object_id}")
                continue
